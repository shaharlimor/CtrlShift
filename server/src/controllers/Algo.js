...
const scheduleShifts = async (monthlyShifts, users, constraints) => {
  ...
  const assignShiftToEmployee = async (shiftID, roleType, employeeID) => {
    if (employeeID == null) {
      ...
      console.log(
        `user ${employeeID} can do shift ${shiftID} in role ${roleType}`
      );
  ...
  const chooseEmployee = (shift) => {
    if (shift.possibleEmployees.length == 1) {
      ...
      return shift.possibleEmployees.reduce(
        (min, user) => {
          ...
          if (userWeight.given > perfectAmount) {
            weight =
              userWeight.given +
              userWeight.potential / 2 +
              10 * (userWeight.given - perfectAmount);
          } else {
          ...
          const possibleEmployees = users.filter((user) => {
            const hasConstraint = constraints.some(
              (constraint) =>
                ...
                constraint.shiftId === shift._id.toString()
            );
    ...
    for (let i = shifts.length - 1; i >= 0; i--) {
      ...
      for (let j = theShifts.shifts.length - 1; j >= 0; j--) {
        ...
        if (user.role_types.includes(theShift.roleType)) {
          if (
            theShift.possibleEmployees.some(
              (employee) => employee._id === user._id
            )
          ) {
            if (
              theShift.startTime.toISOString().split("T")[0] ==
              shift.startTime.toISOString().split("T")[0]
            ) {
              theShift.possibleEmployees = theShift.possibleEmployees.filter(
                (employee) => employee._id !== user._id
              );
              usersWeight.get(user._id).potential -= 1;
  ...
  while (shifts.length != 0) {
    if (shifts[0].length == 0) {
      ...
      assignShiftToEmployee(
        shifts[0].shifts[0].shiftsID,
        shifts[0].shifts[0].roleType,
        chosenEmployee._id
      );
