...
describe('Schedule Routes', () => {
  ...
  describe('GET /', () => {
    ...
    it('should handle errors when getting schedules', async () => {
      ...
      const response = await request(app).get('/');
      expect(response.text).toBe('error occured to get schedules: Error getting schedules');
    });
  });
  ...
  describe('POST /', () => {
    ...
    it('should handle errors when adding a new schedule', async () => {
      ...
      Schedule.prototype.save = jest.fn().mockRejectedValueOnce('Error adding new schedule');

      const response = await request(app).post('/').send(mockSchedule);
      expect(response.text).toBe('error adding new schedule. error: Error adding new schedule');
    });
  });
  ...
  describe('PATCH /startInsertConstraints/', () => {
    it('should change open to constraints', async () => {
      ...
      changeOpenToConstraints.mockResolvedValueOnce('Open changed to constraints');
      ...
      const response = await request(app).patch('/startInsertConstraints/').send(requestBody);
    ...
    it('should handle errors when changing open to constraints', async () => {
      ...
      changeOpenToConstraints.mockRejectedValueOnce('Error changing open to constraints');
      ...
      expect(response.text).toBe('change start insert constraint failed. error: Error changing open to constraints');
    });
  });
  ...
  describe('PATCH /publishBoard/', () => {
    it('should change publish', async () => {
      ...
      const response = await request(app).patch('/publishBoard/').send(requestBody);
    ...
    it('should handle errors when changing publish', async () => {
      ...
      expect(response.text).toBe('Publish board failed. error: Error changing publish');
    });
  });
  ...
  describe('GET /openToConstraints', () => {
    it('should check if board is open to insert constraints', async () => {
      ...
      const response = await request(app).get('/openToConstraints').send(requestBody);
    ...
    it('should handle errors when checking if board is open to insert constraints', async () => {
      ...
      boardOpenToConstraints.mockRejectedValueOnce('Error checking if board is open to insert constraints');
